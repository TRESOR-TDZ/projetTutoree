...
export default function AdminSysteme() {
  ...
  const fetchAdmins = useCallback(async () => {
    try {
      ...
      console.error("Erreur lors du chargement des admins système", err);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFlashMessage("Administrateur système ajouté avec succès");
  ...
  const handleDeleteConfirmed = async () => {
    try {
      await api.delete(`/admin-systeme/destroy/admin-systeme/${adminToDelete}`);
      fetchAdmins();
  ...
  const handleDownloadPdf = async (e) => {
    ...
    try {
      ...
      const filename = `administrateurs_systeme_${new Date().toISOString().slice(0, 10)}.pdf`;
  ...
  // Télécharger Excel avec filtres
  const handleDownloadExcel = async (e) => {
    ...
    try {
      ...
      const filename = `administrateurs_systeme_${new Date().toISOString().slice(0, 10)}.xlsx`;
      fileDownload(res.data, filename);
    } catch (err) {
        ...
        <div className="mb-4">
          <div className="d-flex align-items-center mb-3">
            <div className={`me-3 p-3 rounded-circle ${theme === "dark" ? "bg-primary bg-opacity-25" : "bg-primary bg-opacity-10"}`}>
              <i data-feather="shield" className="text-primary" style={{ width: "24px", height: "24px" }}></i>
            </div>
            <div>
              ...
              <p className={`mb-0 ${theme === "dark" ? "text-light" : "text-muted"}`}>
                Institut de Santé - Administration centrale du système
              </p>
            </div>
          </div>
                  ...
                  <div className="d-flex justify-content-between align-items-center">
                    <div>
                      <h3 className="mb-0 text-success">
                        {admins.filter(admin => admin.status === "Connecté").length}
                      </h3>
                      <small className={theme === "dark" ? "text-light" : "text-muted"}>
                        Connectés
                      </small>
                  ...
                  <div className="d-flex justify-content-between align-items-center">
                    <div>
                      <h3 className="mb-0 text-warning">
                        {admins.filter(admin => admin.status === "En attente").length}
                      </h3>
                      <small className={theme === "dark" ? "text-light" : "text-muted"}>
                        En attente
                      </small>
                        ...
                        <td>
                          <div className="d-flex align-items-center">
                            <div className="bg-primary bg-opacity-10 rounded-circle p-2 me-3">
                              <i data-feather="shield" className="text-primary" style={{ width: "16px", height: "16px" }}></i>
                            </div>
                            <div>
                        ...
                        <td className="text-center">
                          <div className="d-flex gap-1 justify-content-center">
                            <Button
                              ...
                              as="a"
                              href={`/admin-systeme/show/admin-systeme/${admin.id}`}
                            >
                            ...
                            <Button
                              ...
                              title="Modifier"
                              as="a"
                              href={`/admin-systeme/edit/admin-systeme/${admin.id}`}
                            >
                    ...
                    <tr>
                      <td colSpan="6" className="text-center py-5">
                        <div className={`${theme === "dark" ? "text-light" : "text-muted"}`}>
                          <i data-feather="shield" className="mb-3" style={{ width: "48px", height: "48px", opacity: 0.5 }}></i>
                          <div>
