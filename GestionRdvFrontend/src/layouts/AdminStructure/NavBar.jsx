...
export default function AdminStructureNavbar({ toggleSidebar }) {
  ...
  const toggleTheme = () => {
    ...
    localStorage.setItem("admin-structure-theme", newTheme);
    setTheme(newTheme);
  };
  ...
  return (
    <Navbar className="admin-structure-navbar fixed-top" expand={false}>
        ...
        <div className="logo-container">
          ...
          <Navbar.Brand className="navbar-brand mb-0">
            <div className="brand-text">
              ...
              <span className="brand-sub">Administateur Structure</span>
            </div>
          </Navbar.Brand>
        </div>
      </div>
      ...
      <div className="navbar-right">
        ...
        <Button 
          ...
          title={`Passer au thème ${theme === "dark" ? "light" : "sombre"}`}
        >
          ...
          >
            ...
            <div className="user-info d-none d-sm-block">
              <span className="user-name">
                {truncateName(user?.name || "Admin Structure")}
              </span>
            </div>
          ...
          <Dropdown.Menu className="">
            <div className="dropdown-header">
              <div className="user-details">
                <div className="fw-semibold">{user?.name || "Admin Structure"}</div>
                <div className="text-muted small">{user?.email || "admin@structure.com"}</div>
                <div className="text-primary small">{user?.structure?.nom || "Structure Médicale"}</div>
              </div>
            </div>
            
            <Dropdown.Item 
              as={Link} 
              to="/admin-structure/profil" 
